			...
			<v-row justify='center' class='mt-2' no-gutters>
				<v-col class='text-center mb-4'>
					<v-btn color='red'
						...
						rounded
					>
						<v-icon style='vertical-align: middle;' :color='computedColor' class='mr-2' dark :icon='mdiDeleteSweep' />
...
import type { TAllAlarms, TEmitBody } from '@/types';

const [ alarmStore, piStatusStore, wsStore ] = [ alarmModule(), piStatusModule(), wsModule() ];

const alarms = computed((): TAllAlarms => {
...
const color = computed(() => {
	return piOnline.value ? 'red' : 'grey';
});
