	...
	<section v-if='piTimeValid && piInit'>
		<v-row align='center' class='mr-2' justify='center'>
			<v-col class='pa-0' cols='auto'>
				<div>
					<span
						class='text-white font-weight-medium clock-font'
						:class='[computedFontSize, computedTextColor]'
					>{{ hourExtractor(piTime) }}</span>
					<span class='text-white font-weight-medium' :class='[computedFontSize, computedTextColor]'>:</span>
					<span
						class='text-white font-weight-medium clock-font'
						:class='[computedFontSize, computedTextColor]'
					>{{ minuteExtractor(piTime) }}</span>
					<span class='text-white font-weight-medium' :class='[computedFontSize, computedTextColor]'>:</span>
					<span
						class='text-white font-weight-medium clock-font'
						:class='[computedFontSize, computedTextColor]'
					>{{ secondExtractor(piTime) }}</span>
				</div>
			</v-col>

			<v-col class='pa-0 mx-2' cols='auto'>
				<v-icon
					class='mb-1'
...
const piTimeValid = computed(() => !Number.isNaN(piStatusStore.time.hours))
