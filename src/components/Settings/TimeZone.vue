	...
	<v-col cols='12' class=''>
		<v-row align='center' justify='center' class='no-gutters mb-2'>
			<v-col cols='auto' class='pa-0'>
				<v-btn @click='visibility' :color='infoColor' :dark='!loading' :disabled='localLoading || loading'
			...
			<section v-if='visible'>
				...
				<v-form v-on:submit.prevent autocomplete='off' method='post'>
					<v-row align='center' justify='space-around' no-gutters>
						<v-col cols='10' sm='5' class=''>
							<v-select v-model='timeZoneRegion' :disabled='!piOnline || localLoading' :items='parents'
								density='compact'
								color='primary' label='Region' variant='outlined' />
						</v-col>
						<v-col cols='10' sm='5' class=''>
							<v-select v-model='timeZoneCity' :disabled='!piOnline || localLoading' :items='computedCity'
								density='compact'
								color='primary' item-text='text' item-value='value' label='City'
						...
						<v-col cols='auto'>
							<v-row justify='space-around' class=''>
								<v-col cols='auto' class=''>
									<v-btn @click='visibility' :dark='!localLoading' :disabled='localLoading'
										:size='mobile ? `small` : `default`' class='elevation-0' color='red' rounded
										variant='outlined'>
										<span>
											<v-icon size='small' style='vertical-align: middle;' class='mr-2'
												:icon='mdiClose' />
											cancel
										</span>
									</v-btn>
								...
								<v-col cols='auto' class=''>
									<v-btn @click='setTimeZone' :disabled='localLoading || !timeZoneCity'
										:size='mobile ? `small` : `default`' class='elevation-0' color='success'
										:variant='!updateDisabled ? "outlined" : "flat"' rounded>
										<v-icon :color='!updateDisabled ? "black" : "white"'
											style='vertical-align: middle;' class='mr-1' :icon='mdiUpdate' />
